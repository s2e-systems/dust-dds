use super::{overall_structure::IntoBytes, submessage_elements::{ParameterList, Parameter}};
...
impl IntoBytes for EntityId {
    fn into_bytes<E: byteorder::ByteOrder>(&self, buf: &mut [u8]) -> usize {
        self.entity_key().into_bytes::<E>(&mut buf[0..]);
        self.entity_kind().into_bytes::<E>(&mut buf[3..]);
        4
    }
}

impl IntoBytes for SequenceNumber {
    fn into_bytes<E: byteorder::ByteOrder>(&self, buf: &mut [u8]) -> usize {
...
impl IntoBytes for Parameter  {
    fn into_bytes<E: byteorder::ByteOrder>(&self, mut buf: &mut [u8]) -> usize {
...
impl IntoBytes for ParameterList {
    fn into_bytes<E: byteorder::ByteOrder>(&self, buf: &mut [u8]) -> usize {
        let mut length = 0;
        for parameter in self.parameter().iter() {
            length += parameter.into_bytes::<E>(&mut buf[length..]);
        }
