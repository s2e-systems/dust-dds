...
use crate::implementation::{
    rtps::messages::{overall_structure::RtpsSubmessageHeader, RtpsSubmessageWriteKind, RtpsSubmessageReadKind, RtpsMessageRead, RtpsMessageWrite,},
...
impl MappingWriteByteOrderInfoInData for RtpsSubmessageWriteKind<'_> {
    fn mapping_write_byte_order_info_in_data<W: Write>(&self, mut writer: W) -> Result<(), Error> {
        match self {
            ...
            RtpsSubmessageWriteKind::Data(s) => s.mapping_write_byte_order_info_in_data(&mut writer)?,
            RtpsSubmessageWriteKind::DataFrag(s) => {
                s.mapping_write_byte_order_info_in_data(&mut writer)?
            }
            RtpsSubmessageWriteKind::Gap(s) => s.mapping_write_byte_order_info_in_data(&mut writer)?,
            ...
            RtpsSubmessageWriteKind::Pad(s) => s.mapping_write_byte_order_info_in_data(&mut writer)?,
        };
        Ok(())
    }
}
...
//                 submessages::{HeartbeatSubmessage},
