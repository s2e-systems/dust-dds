...
use dds_api::{
    ...
    dcps_psm::{BuiltInTopicKey, ANY_SAMPLE_STATE, ANY_VIEW_STATE},
...
pub fn task_spdp_discovery(
    ...
    if let Ok(samples) =
        spdp_builtin_participant_data_reader.take(1, ANY_SAMPLE_STATE, ANY_VIEW_STATE, &[])
    {
...
pub fn task_sedp_writer_discovery(
    ...
    let samples = sedp_builtin_publication_reader.take(1, ANY_SAMPLE_STATE, ANY_VIEW_STATE, &[]);
...
pub fn task_sedp_reader_discovery(
    ...
    let samples = sedp_builtin_subscription_reader.take(1, ANY_SAMPLE_STATE, ANY_VIEW_STATE, &[]);
