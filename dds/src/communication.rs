...
use rust_dds_rtps_implementation::{
    dds_impl::{
        data_writer_proxy::{RtpsWriter, DataWriterAttributes}, publisher_proxy::PublisherAttributes,
        subscriber_proxy::SubscriberAttributes, data_reader_proxy::DataReaderAttributes,
    },
...
where
    T: TransportWrite,
{
    pub fn send(&mut self, data_writer: &mut RtpsShared<DataWriterAttributes<RtpsStructureImpl>>) {
        let mut rtps_writer_lock = data_writer.write_lock();
    ...
    pub fn send_with_publishers(&mut self, list: &[RtpsShared<PublisherAttributes<RtpsStructureImpl>>]) {
...
where
    T: TransportRead,
{
    pub fn receive(&mut self, data_reader: &RtpsShared<DataReaderAttributes<RtpsStructureImpl>>) {
    ...
    pub fn receive_with_subscribers(&mut self, list: &[RtpsShared<SubscriberAttributes<RtpsStructureImpl>>]) {
        if let Some((source_locator, message)) = self.transport.read() {
            MessageReceiver::new().process_message_with_subscribers(
                self.guid_prefix,
                list,
                source_locator,
                &message,
