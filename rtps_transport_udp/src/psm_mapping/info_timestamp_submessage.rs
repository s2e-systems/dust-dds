use rust_rtps::messages::submessages::{InfoTs, SubmessageHeader};
use rust_rtps::messages::types::constants::TIME_INVALID;

use super::submessage_elements::{deserialize_timestamp, serialize_timestamp};
use super::UdpPsmMappingResult;

pub fn serialize_info_timestamp(
    info_timestamp: &InfoTs,
...
pub fn deserialize_info_timestamp(
    bytes: &[u8],
    header: SubmessageHeader,
) -> UdpPsmMappingResult<InfoTs> {
    ...
    /*I*/
    let invalidate_flag = flags[1];

    let endianness = endianness_flag.into();
    if invalidate_flag {
        Ok(InfoTs::from_raw_parts(
            endianness_flag,
            invalidate_flag,
            TIME_INVALID,
        ))
...
mod tests {
    ...
    fn test_serialize_deserialize_infots() {
        ...
        let info_timestamp_message_little_endian = [0xB1, 0x05, 0x50, 0x5D, 0x43, 0x22, 0x11, 0x10]; // 0x09, 0x01, 0x08, 0x00,
        let info_timestamp_message_big_endian = [0x5D, 0x50, 0x05, 0xB1, 0x10, 0x11, 0x22, 0x43]; //0x09, 0x00, 0x00, 0x08,

        let test_time = rust_rtps::messages::types::Time::new(1565525425, 269558339);
        ...
        assert_eq!(
            deserialize_info_timestamp(
                &writer,
                infots_big_endian.submessage_header(info_timestamp_message_big_endian.len() as u16)
            )
            .unwrap(),
            infots_big_endian
        );
        ...
        assert_eq!(
            deserialize_info_timestamp(
                &writer,
                infots_little_endian
                    .submessage_header(info_timestamp_message_little_endian.len() as u16)
            )
            .unwrap(),
            infots_little_endian
        );
