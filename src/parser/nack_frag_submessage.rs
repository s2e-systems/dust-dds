...
pub fn parse_nack_frag_submessage(submessage: &[u8], submessage_flags: &u8) -> Result<NackFrag> {
    ...
    let writer_sn : SequenceNumber = deserialize::<SequenceNumberSerialization>(submessage, &SEQUENCE_NUMBER_FIRST_INDEX, &SEQUENCE_NUMBER_LAST_INDEX, &submessage_endianess)?.into();
    
    let (fragment_number_set, fragment_number_set_size) = parse_fragment_number_set(submessage, &FRAGMENT_NUMBER_SET_FIRST_INDEX, &submessage_endianess)?;
...
mod tests{
    ...
    fn test_parse_nack_frag_submessage_big_endian() {
        ...
        assert_eq!(nack_frag_submessage.fragment_number_set,
            vec!( (4113, true), (4114,true), (4115, true), (4116, true), (4117, false),
                  (4118, false), (4119,false), (4120, false), (4121, true), (4122, true)));
