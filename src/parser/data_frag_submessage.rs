...
pub fn parse_data_frag_submessage(submessage: &[u8], submessage_flags: &u8) -> Result<DataFrag> {
    ...
    let fragment_starting_num: FragmentNumber = deserialize::<FragmentNumber>(submessage, &FRAGMENT_STARTING_NUMBER_FIRST_INDEX, &FRAGMENT_STARTING_NUMBER_LAST_INDEX, &submessage_endianess)?;

    let fragments_in_submessage: u16 = deserialize::<u16>(submessage, &FRAGMENTS_IN_SUBMESSAGE_FIRST_INDEX, &FRAGMENTS_IN_SUBMESSAGE_LAST_INDEX, &submessage_endianess)?;

    let fragment_size: u16 = deserialize::<u16>(submessage, &FRAGMENT_SIZE_FIRST_INDEX, &FRAGMENT_SIZE_LAST_INDEX, &submessage_endianess)?;

    let sample_size: u32 = deserialize::<u32>(submessage, &SAMPLE_SIZE_FIRST_INDEX, &SAMPLE_SIZE_LAST_INDEX, &submessage_endianess)?;
