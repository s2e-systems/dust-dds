...
int main(void) {
    ...
    ReturnCode result = dds_status_condition_set_enabled_statuses(reader_cond, DUST_DDS_STATUS_SUBSCRIPTION_MATCHED_STATUS);
    assert(result == RETCODE_OK);

    DustDdsWaitSet* wait_set = dds_wait_set_new();
    assert(wait_set != NULL);
    result = dds_wait_set_attach_condition(wait_set, reader_cond);
    assert(result == RETCODE_OK);

    printf("Subscriber waiting for discovery...\n");
    DustDdsDuration wait_timeout1 = { 60, 0 };
    result = dds_wait_set_wait(wait_set, wait_timeout1);
    assert(result == RETCODE_OK);
    ...
    result = dds_wait_set_wait(wait_set, wait_timeout2);
    assert(result == RETCODE_OK);
