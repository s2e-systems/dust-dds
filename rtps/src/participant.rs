...
use rust_dds_interface::protocol::{ProtocolSubscriber, ProtocolPublisher};
...
impl Participant {
    ...
    pub fn create_publisher(&mut self) ->  Box<dyn ProtocolPublisher> {
    ...
    pub fn create_subscriber(&mut self) -> Box<dyn ProtocolSubscriber> {
        let guid_prefix = self.participant.entity.guid.prefix();
    ...
    pub fn reset_discovery(&mut self) {
        self.spdp.spdp_builtin_participant_writer().unsent_changes_reset()
    }
}
