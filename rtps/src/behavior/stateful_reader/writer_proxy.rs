...
impl WriterProxy {
    ...
    pub fn available_changes_max(&mut self) -> SequenceNumber {
        ...
        let lowest_before_missing = self
            .changes_from_writer
            .missing_changes
            .iter()
            .next()
            .map_or(
                self.changes_from_writer.highest_processed_sequence_number,
                |x| *x - 1,
            );

        let lowest_unknown = self
            .changes_from_writer
            .unknown_changes
            .iter()
            .next()
        ...
        self.changes_from_writer
            .highest_processed_sequence_number
            .min(lowest_unknown)
            .min(lowest_before_missing)
    }
    ...
    pub fn lost_changes_update(&mut self, first_available_seq_num: SequenceNumber) {
        let remaining_unknown = self
            .changes_from_writer
            .unknown_changes
            .split_off(&first_available_seq_num);
        self.changes_from_writer
            .lost_changes
            .append(&mut self.changes_from_writer.unknown_changes);
        self.changes_from_writer.unknown_changes = remaining_unknown;

        let remaining_missing = self
            .changes_from_writer
            .missing_changes
            .split_off(&first_available_seq_num);
        self.changes_from_writer
            .lost_changes
            .append(&mut self.changes_from_writer.missing_changes);
        ...
        if first_available_seq_num > self.changes_from_writer.highest_processed_sequence_number {
            for seq_num in self.changes_from_writer.highest_processed_sequence_number + 1
    ...
    pub fn missing_changes_update(&mut self, last_available_seq_num: SequenceNumber) {
        let remaining_unknown = self
            .changes_from_writer
            .unknown_changes
            .split_off(&last_available_seq_num);
        self.changes_from_writer
            .missing_changes
            .append(&mut self.changes_from_writer.unknown_changes);
        self.changes_from_writer.unknown_changes = remaining_unknown;
...
mod tests {
    ...
    fn received_changes_set_unordered() {
        ...
        assert_eq!(writer_proxy.available_changes_max(), 0);
        assert_eq!(
            writer_proxy.changes_from_writer.unknown_changes,
    ...
    fn lost_changes_only() {
        ...
        assert_eq!(writer_proxy.available_changes_max(), 2);
        assert_eq!(
            writer_proxy.changes_from_writer.lost_changes,
            [1, 2].iter().cloned().collect()
        );
    ...
    fn missing_changes_only() {
        ...
        assert_eq!(writer_proxy.available_changes_max(), 0);
        assert_eq!(
            writer_proxy.changes_from_writer.missing_changes,
