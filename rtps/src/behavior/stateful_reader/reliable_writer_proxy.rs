...
impl ReliableWriterProxyBehavior {
    pub fn try_process_message<H: HistoryCache>(
    ...
    fn transition_t8<H: HistoryCache>(writer_proxy: &mut WriterProxy, history_cache: &mut H, data: Data) {
    ...
    fn waiting_heartbeat_state(writer_proxy: &mut WriterProxy, submessage: &RtpsSubmessage) {
    ...
    fn must_send_ack_state(
        ...
        if duration_since_heartbeat_received > heartbeat_response_delay {
            Self::transition_t5(writer_proxy, reader_entity_id, output_queue)
        }
    }
