...
impl BestEffortStatelessReaderBehavior {
    ...
    fn waiting_state(
        ...
        if let Some(inner_submessage) = submessage {
            if let RtpsSubmessage::Data(data) = inner_submessage {
                if reader.endpoint.entity.guid.entity_id() == data.reader_id()
    ...
    fn transition_t2(reader: &mut StatelessReader, guid_prefix: GuidPrefix, data: Data) {
