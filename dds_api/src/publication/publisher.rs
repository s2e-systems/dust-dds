...
use crate::{domain::DomainParticipantListener, types::DDSType, subscription::Subscriber};
use crate::infrastructure::status::StatusMask;
use crate::domain::DomainParticipant;
use crate::topic::{Topic, TopicListener};
...
use crate::types::{ReturnCode, Duration, InstanceHandle, ReturnCodes};
use crate::infrastructure::qos::{TopicQos, PublisherQos, DataWriterQos, DomainParticipantQos};
...
pub trait Publisher : DomainEntity<Qos=PublisherQos, Listener=Box<dyn PublisherListener>>{
    ...
    fn create_datawriter<T: DDSType>(
        &self,
        a_topic: &dyn Topic<T, Qos=TopicQos, Listener=dyn TopicListener<T>>,
    ...
    fn delete_datawriter<T: DDSType>(
        &self,
        a_datawriter: &mut dyn DataWriter<T>
    ) -> ReturnCode<()>;
    ...
    fn get_participant<T: DomainParticipant>(&self) -> &T;
