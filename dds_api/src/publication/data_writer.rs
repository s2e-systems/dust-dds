...
use crate::types::{InstanceHandle, Time, ReturnCode, Duration, ReturnCodes, ChangeKind};
use crate::infrastructure::qos::{PublisherQos, TopicQos, DataWriterQos};
...
use crate::publication::PublisherListener;
...
impl<T:DDSType> AnyDataWriter for dyn DataWriter<T>
{}

pub trait DataWriter<T: DDSType> : DomainEntity<Qos=DataWriterQos, Listener=Box<dyn DataWriterListener<T>>> {
    ...
    // fn get_publisher<P: Publisher>(&self,) -> &P;
