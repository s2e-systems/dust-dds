use rust_dds_types::{DDSType, Duration, InstanceHandle, ReturnCode, Time};
...
pub trait DataWriter<'a, T: DDSType>:
    Entity<'a, Qos = DataWriterQos, Listener = Box<dyn DataWriterListener<T>>>
{
    ...
    fn register_instance(&self, instance: T) -> ReturnCode<Option<InstanceHandle>>;
    ...
    fn unregister_instance(&self, instance: T, handle: Option<InstanceHandle>) -> ReturnCode<()>;
    ...
    /// unspecified.
    fn get_key_value(&self, key_holder: &mut T, handle: InstanceHandle) -> ReturnCode<()>;
    ...
    fn lookup_instance(&self, instance: &T) -> ReturnCode<Option<InstanceHandle>>;
    ...
    fn write(&self, data: T, handle: Option<InstanceHandle>) -> ReturnCode<()>;
    ...
    /// This operation may return OUT_OF_RESOURCES under the same circumstances described for the write operation
    /// (2.2.2.4.2.11).
    fn dispose(&self, data: T, handle: Option<InstanceHandle>) -> ReturnCode<()>;
    ...
    fn get_liveliness_lost_status(&self, status: &mut LivelinessLostStatus) -> ReturnCode<()>;
